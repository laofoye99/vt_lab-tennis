...
class PersonDetector(SharedData):
    def __init__(self):
        super().__init__()
        self.model = YOLO('/home/max/Desktop/tennis/tennis-v5/notebook/yolo11l.pt')
        self.min_score = 0.3
        self.player1_boxes = []
        self.player2_boxes = []
    ...
    def detect(self, frame, event_draw=False):
        ...
        if event_draw:
            cv2.rectangle(frame, (int(p1[0]), int(p1[1])), (int(p1[2]), int(p1[3])), (0,255,0), 2)
            cv2.rectangle(frame, (int(p2[0]), int(p2[1])), (int(p2[2]), int(p2[3])), (0,255,0), 2)
